...
triggers: change order, pending decision, ccb, baseline, scope creep, co-
tags: governance, change-control, ccb, baseline
---

## The purpose is baseline integrity
...
## Undecided is the expensive state

A change order sitting in *Requested* or *Under Review* is not neutral. The team is usually already doing the work — that is why it was raised — so the cost is being incurred against an unfunded baseline. The longer it sits, the more of the change is delivered before anyone decides whether to fund it.
...
## What a defensible change order carries

- The **driver** — client request, differing site condition, regulatory shift, error. Drivers determine who pays.
...
## Approval is a CCB act

Approval authority belongs to the change control board, at the level matching the threshold. It is not a project-manager convenience and it is not something an automated assistant should perform. Tools may *draft* a change order; only the authorised body approves one.
...
- Track baseline-versus-current explicitly so approved change is visible as variance rather than absorbed into the original number.

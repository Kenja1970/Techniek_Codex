...
- **Lead magnet** branch `feat/lead-magnet` (repo Techniek_Codex): homepage "Project Readiness
  Guide" gated form (first name / email / optional phone) in `outputs/index.html` posting to a
  configurable form backend (placeholder Formspree endpoint `REPLACE_WITH_YOUR_FORM_ID` — SWAP IN
  the real endpoint to enable lead delivery; until then it delivers the PDF client-side).
  `gregory@techniekengineering.com` wired into contact actions + footer. Guide PDF generated by
  zero-dep `tools/generate-lead-magnet.mjs` → `outputs/assets/techniek-project-readiness-guide.pdf`.
  Added root `.gitattributes` marking binaries (incl. `*.pdf`) as binary. `npm test` passes.
...
STILL TODO (flange deep work, not started): flange real published B16.5/B16.47 nominal-dimension +
P-T dataset (provenance-marked); fuller VIII-2 §4.16 + ISO 27509 compact calcs; numeric
`calculate()` tests; extract the tool into a new `Kenja1970/Techniek-FlangeCapacity` repo. Lead
magnet (item 4) is DONE except swapping the placeholder Formspree endpoint for the real one.
